...
class WelcomeController extends Controller
{
    ...
    public function searchIncident($code = null)
    {
    ...
    public function apiincidentRegister()
    {
        // Obtener los tipos de incidentes de la base de datos
        $incidentTypes = IncidentType::pluck('name', 'id')->toArray();

        // Obtener los incidentes
        $incidents = Incident::all();

        // Extraer los años únicos
        $years = $incidents->pluck('created_at')
            ->map(fn($date) => (new DateTime($date))->format('Y'))
            ->unique()
            ->sort()
            ->values()
            ->toArray();
        ...
        $incidentCounts = array_fill_keys($years, array_merge(['total' => 0], array_fill_keys($incidentTypes, 0)));

        // Contar incidentes
        foreach ($incidents as $incident) {
            $year = (new DateTime($incident->created_at))->format('Y');
            $incidentCounts[$year]['total']++;

            if (isset($incidentTypes[$incident->incident_type_id])) {
                $incidentCounts[$year][$incidentTypes[$incident->incident_type_id]]++;
            }
        }

        // Devolver datos en formato adecuado
        return response(array_merge(
            [array_values($years)], // Años
            [array_column($incidentCounts, 'total')], // Total incidentes
            array_map(fn($type) => array_column($incidentCounts, $type), array_values($incidentTypes)), // Tipos de incidentes
            [array_values($incidentTypes)] // Nombres de los incidentes
        ));
    }
}
