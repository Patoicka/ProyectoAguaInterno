...
class IncidentController extends Controller
{
    ...
    //Funtion and route to view incidents and filter by municipality, state, type, status and date.
    public function viewIncidents()
    {
        return Inertia::render('ViewIncidents/SeeIncidents', [
            'title' => 'Incidencias Generales',
            'routeName' => $this->routeName,
        ]);
    }

    //Obtains the total number of incidents by municipality
    public function byMunicipality()
    {

        $datos = Incident::join('locations', 'incidents.location_id', '=', 'locations.id')
            ->join('neighborhoods', 'locations.neighborhood_id', '=', 'neighborhoods.id')
            ->join('cities', 'neighborhoods.city_id', '=', 'cities.id')
            ->select('cities.name as municipio', DB::raw('count(*) as total'))
            ->whereNull('incidents.deleted_at')
            ->groupBy('cities.name')
            ->get();

        $chartData = [
            'labels' => $datos->pluck('municipio'),
            'datasets' => [
                [
        ...
        return Inertia::render('ViewIncidents/ByMunicipality', [
            'title' => 'Incidencias por Municipio',
    ...
    {
        $datos = DB::table('incidents')
            ->join('locations', 'incidents.location_id', '=', 'locations.id')
            ->join('neighborhoods', 'locations.neighborhood_id', '=', 'neighborhoods.id')
            ->join('cities', 'neighborhoods.city_id', '=', 'cities.id')
            ->join('states', 'cities.state_id', '=', 'states.id')
            ->select('states.name as estado', DB::raw('count(*) as total'))
            ->whereNull('incidents.deleted_at')
            ->groupBy('states.name')
            ->get();
        ...
        return Inertia::render('ViewIncidents/ByState', [
            'title' => 'Incidencias por Estado', // Título dinámico
            'barChartData' => $chartData,
        ]);
    }
...
{
    $datos = Incident::join('incident_types', 'incidents.incident_type_id', '=', 'incident_types.id')
        ->select('incident_types.name as tipo', DB::raw('count(*) as total'))
        ->whereNull('incidents.deleted_at')
    ...
    return Inertia::render('ViewIncidents/ByType', [
        'title' => 'Incidencias por Tipo', // Título dinámico
        'barChartData' => $chartData,
    ]);
}
...
{
    $datos = Incident::join('incident_statuses', 'incidents.incident_status_id', '=', 'incident_statuses.id')
        ->select('incident_statuses.name as status', DB::raw('count(*) as total'))
        ->whereNull('incidents.deleted_at')
        ->groupBy('incident_statuses.name')
        ->get();

    $chartData = [
        'labels' => $datos->pluck('status'),
        'datasets' => [
            [
                'label' => 'Total por Estatus',
                'data' => $datos->pluck('total'),
    ...
    return Inertia::render('ViewIncidents/ByStatus', [
        'title' => 'Incidencias por Estatus', // Título dinámico
        'barChartData' => $chartData,
    ]);
}
...
{
    $datos = Incident::select(DB::raw('DATE(created_at) as fecha'), DB::raw('count(*) as total'))
    ...
    return Inertia::render('ViewIncidents/ByDate', [
        'title' => 'Incidencias por Fecha', // Título dinámico
