...
import { defineProps, ref, computed, watch, provide, onMounted, watchEffect,onBeforeUnmount } from "vue";
...
//const siteKey = "6Le8-QArAAAAAMk3IJj7j0ub2NSRnhr65PT0P9dy"; // clave de reCAPTCHA
...
const loadRecaptcha = () => {
  // Si ya está cargado, renderiza inmediatamente
  ...
  // Si el script ya está en el DOM pero grecaptcha no está listo
  ...
  script.defer = true;
  script.onerror = () => {
    recaptchaError.value = "Error al cargar reCAPTCHA";
  };
  document.head.appendChild(script);
}; */

/* Renderizado de reCAPTCHA
const renderRecaptcha = () => {
  if (!window.grecaptcha || typeof window.grecaptcha.render !== 'function') {
    console.error("reCAPTCHA API no está disponible");
    recaptchaError.value = "reCAPTCHA no está listo";
    return;
  }

  const container = document.getElementById("recaptcha-container");
  if (!container) {
    console.warn("Contenedor de reCAPTCHA no encontrado");
    return;
  }
  ...
  try {
    recaptchaWidgetId.value = window.grecaptcha.render(container, {
      ...
      'error-callback': () => {
        form.recaptcha = null;
        recaptchaError.value = "Error en la verificación";
      }
    });
  } catch (error) {
...
/* Validación y envío
const validateRecaptchaAndSubmit = async () => {
  try {
    if (!window.grecaptcha || recaptchaWidgetId.value === null) {
      alert("reCAPTCHA no está listo. Por favor, espera un momento.");
      return;
    }

    const response = grecaptcha.getResponse(recaptchaWidgetId.value);
    if (!response) {
    ...
    console.log("Token reCAPTCHA:", response);
    await saveForm();
  } catch (error) {
...
/* Resetear al desmontar si es necesario
onBeforeUnmount(() => {
  if (window.grecaptcha && recaptchaWidgetId.value !== null) {
                ...
                <template #footer>
                    <BaseButtons>
                        ...
                        <div class="mt-4 flex justify-center">
                            <FormField label="reCAPTCHA" required :error="form.errors.recaptcha || recaptchaError">
                                <div id="recaptcha-container" class="p-4 bg-gray-100 rounded-lg shadow-md"></div>
                            </FormField>
                        </div> -->
            </CardBox>
        </div> 
