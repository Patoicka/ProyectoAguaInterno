...
const { filters, clearFilters, applyFilters } = useFilters(props.filters, props.routeName, isLoading);
const headers = ['Nombre', 'Descripción', 'Fecha', 'Estatus'];
const values = ['name', 'description', 'status', (item) => dateToLocal(item.created_at, true)]
...
<template>
    ...
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            v-model:rows="filters.rows" :routeName="routeName" title="tipo" />
                ...
                <tbody>
                    <tr v-for="item in incidentTypes.data" :key="item.id">
                        ...
                        <td data-label="Acciones">
                            <BaseButtons>
                                <BaseButton v-if="useCan('incidentType.update')" color="info" :icon="mdiPencil" small
                                    :routeName="`${routeName}edit`" :parameter="item.id" title="Editar tipo" />
                            </BaseButtons>
                        </td>
                    </tr>
                </tbody>
        ...
        <pagination :links="incidentTypes.links" :total="incidentTypes.total" :to="incidentTypes.to" :from="incidentTypes.from" />
    </LayoutMain>
</template>